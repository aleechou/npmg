...
function targetResolver (where, context, deps) {
	...
	cache.add(what, function (er, data) {
	    ...
	    if(er && er.message=="invalid tar file" && err.header )
		log.error("can not unpack file from tar:",err.header.path) ;
	    ...
	    if(data) data.originInstallArgv = what ;
...
function installOne_ (target, where, context, cb) {
    ...
    log.verbose('installOne_()',target.name," where: ",where) ;
    ...
    if( target.dependencies && target.dependencies.opencomb )
	targetFolder = npm.dir + '/' + target.name ;
    log.verbose('installOne_()','targetFolder: '+targetFolder) ;
