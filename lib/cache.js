...
function add (args, cb) {
  ...
  switch (p.protocol) {
    ...
    default:
      ...
      if (isGitUrl(p)){
          return addRemoteGit(spec, p, name, false, function(err){
	      cb.apply(this,arguments) ;
	  })
      }

      log.verbose("before addNamed() or addLocal(), name:",name) ;
...
function archiveGitRemote (p, u, co, origUrl, cb) {
  ...
  function next () {
    mkdir(path.dirname(tmp), function (er) {
      ...
      var env = gitEnv()
      cb = once(cb)
	log.verbose( "pack git dir", args.join(' ') ) ;
...
function addNamed (name, x, data, cb_) {
  ...
  log.verbose("addNamed version:", [semver.valid(x), semver.validRange(x)])
...
function addNameTag (name, tag, data, cb_) {
  if (typeof cb_ !== "function") cb_ = data, data = null
  log.verbose("addNameTag", [name, tag])
  var explicit = true
  if (!tag) {
    explicit = false
...
function addNameRange (name, range, data, cb) {
  ...
  log.verbose("addNameRange", {name:name, range:range, hasData:!!data})

  if (data) return next()
  registry.get(name, function (er, d, json, response) {
      log.verbose("registry.get() error:",er) ;
    if (er) return cb(er)
    data = d
    next()
  })
  ...
  function next () {
    log.verbose( "addNameRange", "number 2"
             , {name:name, range:range, hasData:!!data})
    engineFilter(data)

    log.verbose("addNameRange", "versions"
...
function addNameVersion (name, v, data, cb) {
  ...
  function next () {
    ...
    if (!dist.tarball) return cb(new Error(
      ...
      // fetch module from repositor
      /*var repoModules = npm.config.get('repoModules') ;
      if( data.repository && data.repository.type=='git' && repoModules[name] ) {
	  addRemoteGit (data.repository.url, url.parse(data.repository.url), name, false, cb) ;
	  return ;
      }*/
...
function unpack (pkg, ver, unpackTarget, dMode, fMode, uid, gid, cb) {
  ...
  read(pkg, ver, false, function (er, data) {
	...
	// install as repository work dir
	if( npm.config.repo.asWorkdir(pkg) ) {

	    var argvs ;
	    var up = url.parse(data.originInstallArgv)||{} ;
	    if( isGitUrl(up) && data.originInstallArgv){
		argvs = ['clone'] ;
		if(up.hash) {
		    argvs.push( '-b',up.hash.replace(/^#/,'') ) ;
		}
		var u = up.protocol.replace(/^git\+/,'') + "//" + up.hostname + up.path ;
		argvs.push(u,'.') ;
	    }
	    else if( data.repository &&  data.repository.type && data.repository.type.toLowerCase()=='git' && data.repository.url ){
		argvs = ['clone',data.repository.url,'.'] ;
	    }

	    log.verbose('exec',argvs) ;

	    if(argvs) {

		log.http("unpack pkg as a repository work dir",unpackTarget) ;

		return mkdir(unpackTarget, function (er) {
		    ...
		    exec( npm.config.get("git")
			  , argvs
			  , {cwd: unpackTarget, env: env}
			  , function(err,stdout,stderr){
			      if (er) return cb(er) ;
			      log.verbose(
				  "git clone " + data.repository.url
				  ,  (stdout + "\n" + stderr).trim()
			      ) ;
			      cb && cb () ;
			  }
